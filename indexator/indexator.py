...
class Indexator:
    ...
    def iterate_lemmata(self, langID, lemmaFreqs, lemmaDIDs):
        ...
        for l, lID in self.tmpLemmaIDs[langID].items():
            ...
            iLemma += 1
            yield curAction
            # TODO: Add lemma_freq objects.
            # for docID in lemmaDIDs[lID]:
            #     lfreqJson = {
            #         'wtype': 'lemma_freq',
            #         'l_id': 'l' + str(lID),
            #         'd_id': docID,
            #         'l_order': lOrder,
            #         'freq': self.lemmaDocFreqs[langID][(lID, docID)],
            #         'word_join': {
            #             'name': 'lemma_freq',
            #             'parent': 'l' + str(lID)
            #         }
