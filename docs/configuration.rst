...
- ``localize_metadata_values`` (Boolean) -- if set, then metadata values are selected based on the current interface language, if available. When displaying such metadata (including title and author in example headers), Tsakorpus will rename all metadata fields whose names end with ``_%INTERFACE_LANGUAGE_CODE%``, chopping the language code. For example, if there is ``title_en`` in the metadata, and current interface language is ``en``, then the corresponding value will be treated as if it were the value of ``title``. If there was a ``title`` as well, its value will be overwritten. If there are metadata fields with other language codes, such as ``title_fr``, they will be left intact. However, if they are not on the ``viewable_meta`` list, they will not be displayed. So, for example, if you want to display a localized title in a corpus with English and French interfaces, you can fill in ``title_en`` and ``title_fr`` in your documents, but only put ``title`` in ``viewable_meta``.
