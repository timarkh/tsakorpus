...
var img = $("#png_img")[0];
var downloadA = $("#dummy_download_link")[0];
...
$(function() {
    ...
    function assign_word_stats_table_events() {
        ...
        $('.stats_value_selector').unbind('click');
        $('.stats_value_selector').click(toggle_one_stats_value);
    }
    ...
    function show_bar_chart(results, maxHeight) {
        ...
        var barWidth = 20;
        var nBars = nResults;
        if (nBars >= maxStatsValues) {
            nBars = maxStatsValues;
            $('#word_stats_max_results').show();
            $('#word_stats_max_results').addClass("show");
        }
        else {
            $('#word_stats_max_results').hide();
        }
        if (nBars > 25) {
            nBars = 25;
            $('#word_stats_max_bars').show();
            $('#word_stats_max_bars').addClass("show");
        }
        else {
        ...
        if (nBars < nResults) {
            ...
            for (iQueryWord = 0; iQueryWord < results.length; iQueryWord++) {
                results_truncated.push([]);
                results_truncated[iQueryWord] = results[iQueryWord].slice(0, nBars);
            }
...
//                .text();
    ...
    function clear_table() {
        ...
        $('#word_stats_table_header_top').html("<th></th><th></th>");
        ...
        for (iQueryWord = 0; iQueryWord < results.length; iQueryWord++) {
            ...
            for (iRes = 0; iRes < results[iQueryWord].length; iRes++) {
                ...
                if (trExisting.length <= 0) {
                    tr = "<tr id=\"" + rowID + "\"><td><input type=\"checkbox\" class=\"stats_value_selector\" data-nres=\"" + iRes + "\" checked /></td><td>" + v.name + "</td>" + tr + "</tr>";
    ...
    function redraw_word_stats_plot(results) {
        // Redraw the plot after some of the bars have been switched off
        clear_word_stats_plots();
        $('#word_stats_wait').fadeOut();
    ...
    function download_plot_png(e) {
        ...
        img.onload = function () {
            ...
            downloadA.download = "plot.png";
            downloadA.href = dataURL;
            downloadA.dispatchEvent(event);
        }
    }

    function toggle_one_stats_value(e) {
        var clicked_n_res = parseInt($(e.target).attr('data-nres'));
        var res2remove = [];
        $('.stats_value_selector').each(function (index) {
            var nres = parseInt($(this).attr('data-nres'));
            if (!$(this).is(":checked")) {
                res2remove.push(nres);
            }
        });
        data_with_removals = [];
        for (iQueryWord = 0; iQueryWord < data.length; iQueryWord++) {
            data_with_removals.push([]);
            for (iRes = 0; iRes < data[iQueryWord].length; iRes++) {
                if (!res2remove.includes(iRes)) {
                    data_with_removals[iQueryWord].push(data[iQueryWord][iRes]);
                }
            }
        }
        redraw_word_stats_plot(data_with_removals);
    }

    assign_word_stats_events();

    $("#max_stats_values").html(maxStatsValues);
});
