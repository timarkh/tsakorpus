...
class WordformGenerator:
    ...
    def calc_f_0(self, n):
        ...
        while i >= 1:
            sum_of_series += 1/i
            i -= 1
        return round((self.settings['constants']['LENGTH_OF_CORPUS'] - n/2)/sum_of_series)
    ...
    def give_text_repr(self, wordlist, lengths):
        ...
        for i, length in enumerate(lengths):
            lengthvars = [(4 - abs(i)) * [length + i]
                          for i in range(-3, 4) if length + i >= 1]
            lengthvars = [item for sublist in lengthvars for item in sublist]
            le = random.choice(lengthvars)
            text = ''.join(random.choice(self.settings['constants']['ALPHABET'])
                           for i in range(le))
            while text in reprs:
                le = random.choice(lengthvars)
                text = ''.join(random.choice(self.settings['constants']['ALPHABET'])
